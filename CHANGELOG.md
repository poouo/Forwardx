...
- Agent 加密通信稳定性修复：
  - 修复 `/api/stream` 在时间戳超窗时误返回 `500` 的问题，改为返回 `401` 与明确错误信息，触发 Agent 自动校时重试。
  - 时间戳窗口校验新增秒级时间戳兼容（自动换算毫秒），降低旧格式时间戳导致“重放保护误判”的概率。
- 转发规则内核清理兼容性增强：
  - iptables DNAT/SNAT 规则新增 ForwardX 注释标记，清理阶段优先按标记精准删除，减少与其他系统/面板规则相互影响。
  - nftables 端口清理仅删除带 `fwx-rule-` 注释的 DNAT 规则，降低并存环境下误删与高频重复修复导致的额外负载。
...
- 面板版本升级至 `2.3.182`，Agent 目标版本保持 `2.2.114`。
- Android APP 版本保持 `2.3.54`，APK 发布指向保持 `2.3.181`。
