...
export async function getPaymentOrderStats() {
  ...
  const totalOrders = (_sqlite.prepare(`SELECT COUNT(*) AS n FROM payment_orders`).get() as any)?.n || 0;
  const pendingOrders = (_sqlite.prepare(`SELECT COUNT(*) AS n FROM payment_orders WHERE status = 'pending'`).get() as any)?.n || 0;
  const paid = _sqlite.prepare(`SELECT COUNT(*) AS n, COALESCE(SUM(amountCents), 0) AS amount FROM payment_orders WHERE status IN ('paid', 'completed')`).get() as any;
  return {
